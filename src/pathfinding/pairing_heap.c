...
int is_empty(pairing_heap

*pheap) {
return (pheap->root == NULL);
}

void heap_reinit(pairing_heap
...
node get_equivalent_element(pairing_heap
...
void merge_with_root(pairing_heap
...
pheap->
root = merge(pheap->root, hnode);
}
}

heap_node insert(pairing_heap

*pheap,
node *element
) {
heap_node hnode = init_node(element);
merge_with_root(pheap,
&hnode);
set_next(pheap
->AFT->prev, &hnode);
...
void decrease_priority(pairing_heap

*pheap,
heap_node *hnode, node
*element) {
hnode->
element = element;
if (hnode->child != NULL) {
...
pheap->
root = merge(pheap->root, &merged);
make_root(pheap
->root);
}
}

void increase_priority(pairing_heap
...
node remove_from_heap(pairing_heap
